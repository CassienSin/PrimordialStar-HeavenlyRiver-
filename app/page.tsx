    ...
    <main style={{ minHeight: '100vh', background: '#0a0812', color: '#f0e6d3' }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .rows-section { padding: 0 48px 100px; margin-top: -60px; position: relative; z-index: 10; }
        .category-row { margin-bottom: 56px; }
        ...
        .row-count { font-size: 11px; color: rgba(240,230,211,0.2); font-weight: 600; letter-spacing: 1px; }
        .cards-row { display: flex; gap: 14px; overflow-x: auto; padding-bottom: 12px; scrollbar-width: thin; scrollbar-color: rgba(201,168,76,0.3) transparent; }
        .cards-row::-webkit-scrollbar { height: 3px; }
        .cards-row::-webkit-scrollbar-thumb { background: rgba(201,168,76,0.3); border-radius: 2px; }
        .card { flex-shrink: 0; width: 185px; border-radius: 6px; overflow: hidden; background: #16121f; text-decoration: none; display: block; border: 1px solid rgba(201,168,76,0.07); transition: transform 0.3s cubic-bezier(0.34,1.56,0.64,1), box-shadow 0.3s, border-color 0.3s; position: relative; }
        .card:hover { transform: translateY(-6px) scale(1.03); box-shadow: 0 16px 40px rgba(0,0,0,0.6), 0 0 0 1px rgba(201,168,76,0.2), 0 0 20px rgba(201,168,76,0.08); border-color: rgba(201,168,76,0.2); z-index: 2; }
        ...
        @media (max-width: 768px) { .rows-section { padding: 0 16px 80px; } .card { width: 150px; } .card-thumb { width: 150px; height: 90px; } }
      `}</style>
      ...
      {featured ? (
        <HeroPreview video={{ ...featured, thumbnail_url: getStorageUrl(featured.thumbnail_url), video_url: getStorageUrl(featured.video_url) }} />
      ) : (
